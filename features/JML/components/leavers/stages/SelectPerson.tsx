...
import useLeaverEmployeeSearch from "../../../hooks/useLeaverEmployeeSearch";
...
export default function SelectPerson() {
  ...
  const search = useLeaverEmployeeSearch();
  ...
  return (
    <div className="vertical-layout__outer">
      <div className="text-style__small-text text-(--primary-grey)">
        Search for the active staff member who is leaving Powerdeed. Only active
        accounts can be offboarded through this workflow.
      </div>
      ...
      {!employee &&
        ...
        search.search.trim().length >= 2 && (
          <div className="vertical-layout__inner rounded-[10px] border border-(--terciary-grey) p-2.5">
            {search.results.map((candidate, index) => (
              ...
              >
                <div
                  className={`grid h-8 w-8 items-center rounded-[10px] text-center text-style__body--bold ${getRandomClassNameColor(candidate.id)}`}
                >
                ...
                <div className="flex-1">
                  ...
                  <div className="horizontal-layout text-style__small-text text-(--primary-grey)">
                    ...
                    <div>
                      {formatLabel(
                        candidate.employment?.departmentName ??
                          candidate.employment?.departmentId,
                      )}
                    </div>
                  </div>
                </div>
                <Button
                  buttonType="light"
                  buttonText={
                    search.isLoadingSelection ? "Loading..." : "Select"
                  }
      ...
      {employee && (
        <>
          <div className="horizontal-layout rounded-[10px] border border-(--secondary-blue) bg-(--secondary-blue)/10 p-2.5">
            <div
              className={`grid h-8 w-8 items-center rounded-[10px] text-center text-style__body--bold ${getRandomClassNameColor(employee.id)}`}
            >
            ...
            <button
              ...
              className="buttonize rounded-[10px] p-2.5 hover:bg-(--terciary-grey)/30 hover:text-(--primary-red)"
            >
              <FontAwesomeIcon icon={["fas", "xmark"]} />
            </button>
          </div>

          <div className="vertical-layout__inner rounded-[10px] bg-(--terciary-grey)/30 p-2.5">
            <div className="vertical-layout__inner border-b border-(--terciary-grey) pb-2.5">
              ...
              {[
                ...
                [
                  "Manager",
                  employee.employment?.managerName ??
            ...
            <div className="vertical-layout__inner">
              ...
              <div className="horizontal-layout flex-wrap">
                {currentAccess.map((access) => (
                  <div
                    key={access}
                    className="rounded-[10px] border border-(--secondary-blue) bg-(--secondary-blue)/10 px-1 text-style__small-text text-(--secondary-blue)"
                  >
                    {access}
                  </div>
                ))}
          ...
          <Notice tone="warning">
            Continue only after HR or the responsible manager has confirmed this
            exit. Offboarding will revoke active sessions on confirmation.
          </Notice>
        </>
      )}
    </div>
